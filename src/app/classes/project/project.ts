...
export default class Project extends Base {
  /**
   * display main menu for project management
   */
  public async actionShowMainMenu() {
    ...
    menuItems.push({
      ...
      description: "",
      detail: "Flash Micropython firmware using esptool"
    });
    ...
    switch (selectedAction.label) {
      ...
      case "Download Firmware...":
        vscode.commands.executeCommand(
          "vscode.open",
          vscode.Uri.parse("http://www.micropython.org/download")
        );
        break;
    }
  }
  ...
  public async actionNewProject() {
    try {
      ...
      let configFile = path.join(
        workingFolder,
        Base.CONSTANTS.APP.CONFIG_FILE_NAME
      );
        ...
        case "win32":
          selectedPort =
            (await vscode.window.showInputBox({
              ...
              placeHolder: "COM1",
              prompt:
                "Current, we are not support auto detect devices connected for Windows. You need to enter port manually."
            })) || "";
          break;
        default:
          ...
          while (selectedPort === "> Reload list") {
            ...
            selectedPort =
              (await vscode.window.showQuickPick(serialPorts, {
                ignoreFocusOut: true,
      ...
      let configFileObject = {
        ...
        ignore: {
          extensions: [Base.CONSTANTS.APP.CONFIG_FILE_NAME],
          directories: [".vscode"]
        },
        tools: { ampy: "", rshell: "" }
      };
      ...
      if (
        !configFileObject.tools.ampy ||
        !configFileObject.tools.ampy.length ||
        !fs.existsSync(configFileObject.tools.ampy)
      ) {
        if (
          ...
          try {
            ...
            let installResult = await Base.getTerminalHelper().execPromise(
          ...
          try {
            let ampyPath = await Base.getTerminalHelper().execPromise(
              "which ampy"
            );
            configFileObject.tools.ampy = ((ampyPath as string) || "").trim();
          } catch (commandNotFoundException) {
      ...
      try {
        let rshellPath = await Base.getTerminalHelper().execPromise(
          "which rshell"
        );
      ...
      if (
        !configFileObject.tools.rshell ||
        !configFileObject.tools.rshell.length ||
        !fs.existsSync(configFileObject.tools.rshell)
      ) {
        if (
          (await vscode.window.showInformationMessage(
      ...
      fs.writeFileSync(
        path.join(workingFolder, "boot.py"),
        "# This is script that run when device boot up or wake from sleep.\n\n\n"
      );
      fs.writeFileSync(
        path.join(workingFolder, "main.py"),
        '# This is your main script.\n\n\nprint("Hello, world!")\n'
      );
      fs.writeFileSync(configFile, JSON.stringify(configFileObject, null, 2));
      await vscode.commands.executeCommand(
        "vscode.openFolder",
        vscode.Uri.parse(workingFolder)
      );
    } catch (exception) {
      vscode.window.showErrorMessage(
        "Error while preparing new project: " + (exception as Error).message
      );
      this.reportException(exception);
    } finally {
      this.statusDone();
  ...
  public async actionFlashFirmware() {
    if (
      !(await Base.getTerminalHelper().checkExecutableToolPromise(
        "esptool",
        "pip install esptool"
      ))
    ) {
      throw new Error(
        "Required tool not found (`esptool`). Please install `esptool` to run project."
      );
    }
    ...
    let selectedBoard = await vscode.window.showQuickPick(
      ...
      ],
      {
        ignoreFocusOut: true,
        placeHolder:
          "Select your board, current supports ESP32, ESP8266 boards."
      }
    );
    ...
    if (
      (await vscode.window.showInformationMessage(
        path.basename(selectedFirmware) +
          " will be flashed into your " +
          selectedBoard +
          " via " +
          selectedPort +
          ". Continue?",
      ...
      try {
        await this.terminalKillCurrentProcess();
      } catch (e) {
        /* no such process */
      }
      try {
        vscode.window.showInformationMessage(
          "Flashing firmware, please check flashing status..."
        );
        this.statusWarning(true);
        let esptoolCmdPrefix = "esptool --port " + selectedPort + " ";
        this.statusText("Erasing firmware...");
        await Base.getTerminalHelper().execPromise(
          esptoolCmdPrefix + "erase_flash"
        );
        ...
        switch (selectedBoard) {
          ...
          case "[ESP8266] NodeMCU":
            await Base.getTerminalHelper().execPromise(
              esptoolCmdPrefix +
                "--baud 460800 write_flash --flash_size=detect -fm dio 0 " +
                selectedFirmware
            );
            break;
          case "[ESP32] ESP32":
            await Base.getTerminalHelper().execPromise(
              esptoolCmdPrefix +
                "--chip esp32 write_flash -z 0x1000 " +
                selectedFirmware
            );
            break;
        }
        ...
        vscode.window.showErrorMessage(
          "Error while flashing firmware: " + exceptionMessage
        );
        this.reportException(exception);
      } finally {
  ...
  public async actionRun() {
    ...
    vscode.window.showInformationMessage(
      "Start building project files... You can check details in Output window named " +
        Base.CONSTANTS.APP.UNIQUE_NAME +
        "."
    );
    try {
      ...
      if (!vscode.window.activeTextEditor) {
        throw new Error(
          "No active editor can run in this time, please switch to your working file to run!"
        );
      }
      this.outputPrintLn(
        "Preparing file " +
      ...
      if (
        ...
        throw new Error(
          "Required tool not found (`ampy`). Please install `ampy` to run project."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Required tool not found (`rshell`). Please install `rshell` to run project."
        );
      }
      ...
      if (typeof projectConfig !== "object") {
        throw new Error(
          "Error while reading settings from Micropython config file " +
            Base.CONSTANTS.APP.CONFIG_FILE_NAME +
            ". Please recheck config file!"
        );
      }
      ...
      if (!ampyExecutePath || !ampyExecutePath.length) {
        throw new Error(
          "Error while reading `ampy` executable path. Please recheck `tools.ampy` setting inside Micropython config file (`" +
            Base.CONSTANTS.APP.CONFIG_FILE_NAME +
            "`) then try again!"
        );
      }
      ...
      if (!rshellExecutePath || !rshellExecutePath.length) {
        throw new Error(
          "Error while reading `rshell` executable path. Please recheck `tools.rshell` setting inside Micropython config file (`" +
            Base.CONSTANTS.APP.CONFIG_FILE_NAME +
            "`) then try again!"
        );
      }
      ...
      if (!uploadPort || !uploadPort.length) {
        throw new Error(
          "Cannot read port setting in config file. Please recheck `upload.port` setting inside config file " +
            Base.CONSTANTS.APP.CONFIG_FILE_NAME +
            " then try again!"
        );
      }
      ...
      if (!fs.existsSync(uploadPort)) {
        throw new Error(
          "Device at port " +
            uploadPort +
            " is disconnected! Please connect device or re-check your upload port settings in config file: " +
            Base.CONSTANTS.APP.CONFIG_FILE_NAME +
            "."
        );
      }
      ...
      let ignoredExits = ((projectConfig || {}).ignore || {}).extensions || [];
      ...
      let ampyLsResult = (((await Base.getTerminalHelper().execPromise(
        ampyCommandPrefix + "ls"
      )) || "") as string).split("\n");
      for (
        var fileDirIndex = 0;
        fileDirIndex < ampyLsResult.length;
        fileDirIndex++
      ) {
        ...
        if (fileDirPath.length) {
          ...
          try {
            await Base.getTerminalHelper().execPromise(
              ampyCommandPrefix + "rmdir " + fileDirPath
            );
          } catch (e) {
            /* may be item is file */
          }
          try {
            await Base.getTerminalHelper().execPromise(
              ampyCommandPrefix + "rm " + fileDirPath
            );
          } catch (e) {
            /* may be item is path */
          }
      ...
      let resultFiles = this.listFiles(
        projectRoot.uri.fsPath,
        [],
        (filePath: string): boolean => {
          if (fs.statSync(filePath).isFile()) {
            for (
              var ignoredExtIndex = 0;
              ignoredExtIndex < ignoredExits.length;
              ignoredExtIndex++
            ) {
              if (filePath.endsWith(ignoredExits[ignoredExtIndex])) {
                return true;
              }
            }
          } else {
            for (
              var ignoredDirIndex = 0;
              ignoredDirIndex < ignoredDirs.length;
              ignoredDirIndex++
            ) {
      ...
      for (
        ...
        resultFileIndex < resultFiles.length;
        resultFileIndex++
      ) {
        let fileOrDirectoryPath = resultFiles[resultFileIndex];
        let workingPath = fileOrDirectoryPath
          .replace(projectRoot.uri.fsPath, "")
          .replace(projectRoot.uri.fsPath, "")
          .split(" ")
          .join("\\ ");
        try {
          if (fs.statSync(fileOrDirectoryPath).isFile()) {
            let createFileCommand =
              ampyCommandPrefix + "put ." + workingPath + " " + workingPath;
      ...
      this.statusText("[Run] Resetting device...");
      this.outputPrintLn("Resetting state...");
      await Base.getTerminalHelper().execPromise(ampyCommandPrefix + "reset");

      this.outputPrintLn(
        "Task done with " +
          (new Date().getTime() - beginTimeInMillis) +
          " milliseconds."
      );
      vscode.window.showInformationMessage(
        "Building files successfully with " +
          (new Date().getTime() - beginTimeInMillis) +
          " milliseconds!"
      );
      ...
      let errorMessage =
        ...
        "Unknown error while building your project.";
